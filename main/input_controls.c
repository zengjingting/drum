...
static void input_task(void *arg)
{
    ...
    bool s8_raw_pressed = gpio_get_level(PIN_CAPTURE_BUTTON_S8) == 0;
    bool s8_stable_pressed = s8_raw_pressed;
    uint32_t s8_stable_count = 0;

    while (true) {
        const uint8_t current_ab = read_encoder_ab();
        const int8_t detent = encoder_decoder_update(&decoder, current_ab);
        if (detent != 0 && !capture_controller_controls_locked() &&
        ...
        if (stable_count == BUTTON_DEBOUNCE_MS / INPUT_POLL_MS &&
            ...
            stable_pressed = raw_pressed;
            if (stable_pressed && !capture_controller_controls_locked()) {
                metronome_app_toggle();
            }
        }

        const bool s8_pressed = gpio_get_level(PIN_CAPTURE_BUTTON_S8) == 0;
        if (s8_pressed == s8_raw_pressed) {
            if (s8_stable_count < BUTTON_DEBOUNCE_MS / INPUT_POLL_MS) {
                s8_stable_count++;
            }
        } else {
            s8_raw_pressed = s8_pressed;
            s8_stable_count = 0;
        }

        if (s8_stable_count == BUTTON_DEBOUNCE_MS / INPUT_POLL_MS &&
            s8_stable_pressed != s8_raw_pressed) {
            s8_stable_pressed = s8_raw_pressed;
            if (s8_stable_pressed) {
                if (capture_controller_get_state() == CAPTURE_STATE_RECORDING) {
                    (void)capture_controller_stop(CAPTURE_ORIGIN_S8);
                } else {
                    (void)capture_controller_start(CAPTURE_ORIGIN_S8);
                }
            }
        }
...
esp_err_t input_controls_start(void)
{
    ...
    ESP_LOGI(TAG, "Encoder ready: A=%d B=%d button=%d; capture S8=%d",
             PIN_ENCODER_A, PIN_ENCODER_B, PIN_ENCODER_BUTTON,
             PIN_CAPTURE_BUTTON_S8);
    return ESP_OK;
}
